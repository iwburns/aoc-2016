...
fn main() {
    ...
    'outer: for instruction in input {
        ...
        for i in 0..move_amt {
            santa.move_forward(1);

            if santa.locations.contains(&(santa.x, santa.y)) {
                println!("First repeated location: {}", santa.x.abs() + santa.y.abs());
                break 'outer;
            }

            santa.locations.push((santa.x, santa.y));
        }
    }
}
